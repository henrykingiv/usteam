pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: 'access-key', usernameVariable: 'AWS_ACCESS_KEY_ID', passwordVariable: 'AWS_SECRET_ACCESS_KEY')]) {
                        ...
                        // Tag Docker image
                        sh "docker tag ${URL_REGISTRY}/$ECR_REPO:latest ${URL_REGISTRY}/$ECR_REPO:1.01"
        ...
        //     steps {
        //          sh "sleep 90"
        //          sh "curl -s -o /dev/null -w \"%{http_code}\" https://prod.royalhenryking.com"
        //         script {
        //             def response = sh(script: "curl -s -o /dev/null -w \"%{http_code}\" https://prod.royalhenryking.com", returnStdout: true).trim()
        //             if (response == "200") {
